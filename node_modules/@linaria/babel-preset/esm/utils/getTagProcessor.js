...
function getProcessorForIdentifier(path, imports, filename, options) {
  ...
  const tagResolver = options.tagResolver ?? (() => null); // FIXME: can be simplified
  ...
  const [Processor = null, tagPath = null] = relatedImports.map(([imp, p]) => {
...
function getBuilderForIdentifier(path, imports, filename, options) {
  ...
  const astService = { ...t,
    addNamedImport: (name, importedSource) => addNamed(path, name, importedSource)
  };
  return (...args) => new Processor(params, astService, tagPath.node.loc ?? null, replacer, ...args);
}
...
function getDisplayName(path, idx, fileContext) {
  ...
  if (!displayName) {
    const filename = fileContext.filename ?? 'unknown'; // Try to derive the path from the filename
...
export default function getTagProcessor(path, fileContext, options) {
  if (!cache.has(path.node)) {
    ...
    const {
      imports
    } = collectExportsAndImports(root, fileContext.filename);
