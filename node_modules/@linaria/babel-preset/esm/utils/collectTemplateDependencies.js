...
import { findIdentifiers, mutate, reference, referenceAll } from '@linaria/utils';
...
export function extractExpression(ex, evaluate = false, addToExport = true) {
  const {
    loc
  } = ex.node;
  const rootScope = ex.scope.getProgramParent();
  const statementInRoot = ex.findParent(p => p.parentPath?.isProgram() === true);
  const isFunction = ex.isFunctionExpression() || ex.isArrowFunctionExpression(); // Generate next _expN name
  ...
  if (!evaluated) {
    ...
    findIdentifiers([ex], 'referenced').forEach(id => {
      if (!id.isIdentifier()) return; // Try to evaluate and inline them…
  ...
  const kind = isFunction ? ValueType.FUNCTION : ValueType.LAZY; // Declare _expN const with the lazy expression
  ...
  const [inserted] = statementInRoot.insertBefore(declaration);
  referenceAll(inserted);
  rootScope.registerDeclaration(inserted); // Replace the expression with the _expN() call
...
export default function collectTemplateDependencies(path, evaluate = false) {
