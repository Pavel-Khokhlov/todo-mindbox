...
function getProcessorForIdentifier(path, imports, filename, options) {
  ...
  const tagResolver = (_options$tagResolver = options.tagResolver) !== null && _options$tagResolver !== void 0 ? _options$tagResolver : () => null; // FIXME: can be simplified
  ...
  const [Processor = null, tagPath = null] = (_relatedImports$map$f = relatedImports.map(([imp, p]) => {
    const source = tagResolver(imp.source, imp.imported);
    const processor = source ? getProcessorFromFile(source) : getProcessorFromPackage(imp.source, imp.imported, filename);
    return [processor, p];
  }).find(([proc]) => proc)) !== null && _relatedImports$map$f !== void 0 ? _relatedImports$map$f : [];
  return Processor === null || tagPath === null ? [null, null] : [Processor, tagPath];
}

function getBuilderForIdentifier(path, imports, filename, options) {
  const [Processor, tagPath] = getProcessorForIdentifier(path, imports, filename, options);

  if (!Processor || !tagPath) {
    return null;
  }

  const params = [['tag', tagPath.node]];
  let prev = tagPath;
  ...
  const astService = { ..._core.types,
    addNamedImport: (name, importedSource) => (0, _helperModuleImports.addNamed)(path, name, importedSource)
  };
  return (...args) => {
    var _tagPath$node$loc;

    return new Processor(params, astService, (_tagPath$node$loc = tagPath.node.loc) !== null && _tagPath$node$loc !== void 0 ? _tagPath$node$loc : null, replacer, ...args);
  };
}
...
function getDisplayName(path, idx, fileContext) {
  ...
  if (!displayName) {
    ...
    const filename = (_fileContext$filename = fileContext.filename) !== null && _fileContext$filename !== void 0 ? _fileContext$filename : 'unknown'; // Try to derive the path from the filename
...
function isTagReferenced(path) {
  ...
  if (parent) {
    if (parent.isVariableDeclarator()) {
      const id = parent.get('id'); // FIXME: replace with id.isReferencedIdentifier()
...
function getTagProcessor(path, fileContext, options) {
  ...
  if (!cache.has(path.node)) {
    ...
    const {
      imports
    } = (0, _utils.collectExportsAndImports)(root, fileContext.filename);
