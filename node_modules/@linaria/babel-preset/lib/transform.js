...
function syncStages(originalCode, options, prepareStageResults, babelConfig = {}, resolveCache = new Map(), codeCache = new Map(), evalCache = new Map(), eventEmitter) {
  ...
  if (!prepareStageResults || prepareStageResults.every(r => !(0, _withLinariaMetadata.default)(r.metadata))) {
    return {
      code: originalCode,
  ...
  const evalStageResult = (0, _eval.default)(resolveCache, codeCache, evalCache, prepareStageResults.map(r => r.code), options);
  eventEmitter === null || eventEmitter === void 0 ? void 0 : eventEmitter({
  ...
  const collectStageResult = (0, _prepareForRuntime.default)(originalCode, valueCache, options, babelConfig);
  eventEmitter === null || eventEmitter === void 0 ? void 0 : eventEmitter({
...
function transformSync(originalCode, options, syncResolve, babelConfig = {}, resolveCache = new Map(), codeCache = new Map(), evalCache = new Map(), eventEmitter) {
  const {
    filename
  } = options; // *** 1st stage ***
  ...
  const prepareStageResults = (0, _prepareForEval.prepareForEvalSync)(resolveCache, codeCache, syncResolve, entryPoint, options);
  eventEmitter === null || eventEmitter === void 0 ? void 0 : eventEmitter({
...
async function transform(originalCode, options, asyncResolve, babelConfig = {}, resolveCache = new Map(), codeCache = new Map(), evalCache = new Map(), eventEmitter) {
  const {
    filename
  } = options; // *** 1st stage ***
  ...
  const prepareStageResults = await (0, _prepareForEval.default)(resolveCache, codeCache, asyncResolve, entryPoint, options);
  eventEmitter === null || eventEmitter === void 0 ? void 0 : eventEmitter({
    type: 'transform:stage-1:finish',
    filename
  }); // *** The rest of the stages are synchronous ***
